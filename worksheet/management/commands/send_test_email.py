...
SANDBOX_EMAIL_API_KEY = config("SANDBOX_EMAIL_API_KEY", default=None)
SANDBOX_EMAIL_BASE_URL = config(
    "SANDBOX_EMAIL_BASE_URL", default="https://api.mailgun.net"
)
...
def send_simple_email():
    if not all([SANDBOX_EMAIL_API_KEY, SANDBOX_DOMAIN]):
        raise CommandError(
            "EMAIL_API_KEY and SANDBOX_DOMAIN must be set to send the test email."
        )

    url = f"{SANDBOX_EMAIL_BASE_URL.rstrip('/')}/v3/{SANDBOX_DOMAIN}/messages"
    data = {
    ...
    response = requests.post(
        url,
        auth=("api", SANDBOX_EMAIL_API_KEY),
        data=data,
        timeout=10,
    )
    return response
